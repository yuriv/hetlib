...
static void het_container_access(benchmark::State& state) {
  ...
  for (auto _ : state) {
    ...
    values.push_back(1, 1.2f, 3., 'c', "stringview"sv, "string"s);
    state.ResumeTiming();
    auto i = values.fraction<int>().at(k);
    auto f = values.fraction<float>().at(k);
    auto d = values.fraction<double>().at(k);
    auto c = values.fraction<char>().at(k);
    auto sv = values.fraction<std::string_view>().at(k);
    auto s = values.fraction<std::string>().at(k);
    k++;
    // Make sure the variable is not optimized away by compiler
...
static void het_container_match_access(benchmark::State& state) {
  ...
  auto responder = []<>(auto) {return false;};
  // Code inside this loop is measured repeatedly
  for (auto _ : state) {
    state.PauseTiming();
    values.push_back(1, 1.2f, 3., 'c', "stringview"sv, "string"s);
